...
# =========================================================================
# DATATABLES HTML GENERATOR DENGAN COLUMN FILTER (CORONG)
# =========================================================================
def get_datatable_html(df, num_cols, table_id):
    ...
    <head>
        ...
        <style>
            ...
            #{table_id} thead th {{ background-color: #2980b9; color: white; border: 1px solid #555; padding: 10px; text-align: center; position: sticky; top: 0; z-index: 10; }}
            ...
            /* Style khusus untuk corong filter di bawah header */
            thead input {{ width: 100%; padding: 4px; box-sizing: border-box; border-radius: 4px; border: 1px solid #ccc; font-weight: normal; font-size: 11px; }}
        </style>
    </head>
    <body>
    ...
    # Tambahkan baris input untuk kolom filter di bawah Header utama
    html += '<tr class="filter-row">'
    for col in df.columns:
        # Hanya berikan kotak filter untuk kolom non-numerik (string/teks)
        if col not in num_cols:
            html += f'<th><input type="text" placeholder="🔍 Filter {col}" /></th>'
        else:
            html += '<th></th>' # Biarkan kosong untuk kolom angka (Jan, Feb, Total)
    html += "</tr>\n</thead><tbody>"
    ...
    html += f"""
        ...
        <script>
            $(document).ready(function() {{
                ...
                $('#{table_id} thead .filter-row input').on('click keyup', function(e) {{
                    e.stopPropagation();
                }});
