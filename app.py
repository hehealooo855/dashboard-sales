...
def main_dashboard():
    ...
    # Define variables from config to prevent error, now handled by import config
    role = user_role
    my_name = user_name
    ...
    if role in ['manager', 'direktur'] or my_name.lower() == 'fauziah':
        # FIX: Ensure dropping NA and converting to string before sorting
    ...
    with st.sidebar.expander("🔍 Filter Lanjutan", expanded=False):
        # FIX ERROR: Paksa convert ke string dan buang NaN sebelum disort
