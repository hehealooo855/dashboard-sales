...
def render_pivot_fragment(df_scope_all, role):
    ...
    if not master_pivot.empty:
        ...
        if not df_filtered.empty:
            ...
            # ================= IMPLEMENTASI ST_AGGRID (TAB PIVOT) =================
            gb = GridOptionsBuilder.from_dataframe(df_display)
            
            # 1. LOGIKA DEFAULT: NONAKTIFKAN KLIK CORONG
            gb.configure_default_column(sortable=True, resizable=True, suppressMenu=True)

            # 2. LOGIKA FILTER TEXT
            kolom_teks = ['Kode Customer', 'Provinsi', 'Kota', 'Nama Customer']
            for col in kolom_teks:
                if col in df_display.columns:
                    gb.configure_column(col, filter='agSetColumnFilter', floatingFilter=True)

            # 3. LOGIKA FILTER ANGKA
        ...
        with tab_sku:
            ...
            if not filter_nama_sku and not filter_sku_spesifik:
                ...
                if not df_sku_filtered.empty:
                    ...
                    # ================= IMPLEMENTASI ST_AGGRID (TAB SKU) =================
