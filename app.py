...
import datetime
import re # Tambahan untuk pencarian teks canggih
...
def main_dashboard():
    ...
    # Ini data bersih satu perusahaan di tanggal tersebut
    if len(date_range) == 2:
        df_global_period = df[
    ...
    is_supervisor = my_name_key in TARGET_DATABASE # Cek apakah user ini Supervisor (punya target)
    ...
    if role == 'manager':
        ...
        # --- LOGIKA SUPERVISOR (LISMAN, AKBAR, MADONG, WILLIAM) ---
        # Mereka melihat data berdasarkan BRAND, bukan Nama Sales.
        # Ambil daftar brand milik supervisor ini
        my_brands = TARGET_DATABASE[my_name_key].keys()
        
        # Filter Global Data: Ambil semua baris yang Merkenya ada di daftar brand supervisor
        # Kita pakai regex agar pencarian fleksibel (Case Insensitive)
        # Contoh: Target "WhiteLab" akan match dengan data "Whitelab", "WHITELAB"
        brands_pattern = '|'.join([re.escape(b) for b in my_brands])
        
        # Override df_view dengan Data Brand Gabungan Semua Sales
        df_view = df_global_period[df_global_period['Merk'].str.contains(brands_pattern, case=False, na=False)]
        target_sales_filter = my_name # Untuk keperluan label chart
    ...
    if df_view.empty:
        ...
        # --- FITUR KHUSUS MANAGER: TABEL RAPOR SEMUA SUPERVISOR ---
        if role == 'manager' and target_sales_filter == "SEMUA":
            ...
            with st.expander("Klik untuk melihat Detail Target Semua Supervisor", expanded=True):
                ...
                # Loop semua supervisor di database target
                for spv, brands_dict in TARGET_DATABASE.items():
                    for brand, target in brands_dict.items():
                        # Hitung Realisasi Global per Brand (Semua Sales)
                        ...
                        status = "✅" if pct >= 100 else "🔻" # Indikator Merah/Hijau Simpel
                        ...
                        summary_data.append({
                            ...
                            "Target": target,
                            "Realisasi": realisasi,
                            "Persentase": pct / 100, # Format desimal untuk progress bar
                            "Status": status
                        })
                
                df_summary = pd.DataFrame(summary_data)
                
                # Tampilkan Tabel dengan Formatting Warna
                st.dataframe(
                    df_summary,
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "Target": st.column_config.NumberColumn(format="Rp %d"),
        ...
        # --- KPI CARDS & GROWTH ---
        # Hitung Growth (Manual Sederhana)
        prev_omset = 0
        growth_html = ""
        if len(date_range) == 2:
        ...
        with col3:
            ...
            if not df_global_period.empty:
                ...
                # Jika Manager/Supervisor: Lihat Rincian Merk/Sales
                if role == 'manager' or is_supervisor:
                    sales_breakdown = df_view.groupby('Merk')['Jumlah'].sum().reset_index() # Breakdown by Brand
            ...
            if total_target_val > 0:
                ...
                with st.expander("Lihat Rincian Target per Brand", expanded=True):
                    ...
                    for brand, target_brand in active_target_data.items():
                        ...
                        # Indikator Warna Teks di Tabel (Menggunakan Pandas Styler nanti)
                        status_color = "green" if pct >= 100 else "red"
                        
                        brand_data.append({
                            "Brand": brand,
                            "Target": target_brand,
                            "Realisasi": realisasi_brand,
                            "Pencapaian": pct / 100, # Decimal untuk progress bar
                            "Status": "✅ Achieved" if pct >= 100 else "⚠️ On Process"
                        })
                    
                    df_target_breakdown = pd.DataFrame(brand_data)
                    
                    # Style Tabel dengan Warna Background Merah/Hijau pada Kolom Status
                    def highlight_status(val):
                        color = '#d4edda' if 'Achieved' in val else '#f8d7da' # Hijau muda vs Merah muda
                        return f'background-color: {color}; color: black'

                    st.dataframe(
                        df_target_breakdown.style.applymap(highlight_status, subset=['Status']),
                        use_container_width=True, 
                        hide_index=True,
                        column_config={
                            "Target": st.column_config.NumberColumn(format="Rp %d"),
        ...
        # Grafik Tren & Tabel Rincian (Tetap Ada)
