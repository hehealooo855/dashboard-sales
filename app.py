...
import datetime
import time
import re  # Untuk perkuat filter sampah
import bcrypt  # Untuk hash password
import hashlib  # Fallback jika bcrypt tidak tersedia, tapi gunakan bcrypt
...
def load_data():
    # Pindah URL ke secrets untuk security
    url = st.secrets.get("google_sheet_url", "https://docs.google.com/spreadsheets/d/e/2PACX-1vQ4rlPNXu3jTQcwv2CIvyXCZvXKV3ilOtsuhhlXRB01qk3zMBGchNvdQRypOcUDnFsObK3bUov5nG72/pub?gid=0&single=true&output=csv")
    try:
        url_with_ts = f"{url}&t={datetime.datetime.now().timestamp()}"
        df = pd.read_csv(url_with_ts)
        print(f"[DEBUG] Loaded raw data: {len(df)} rows")  # Debug: jumlah row raw
    except Exception as e:
        print(f"[ERROR] Failed to load data: {e}")
        st.warning("Gagal memuat data dari sumber. Coba refresh atau hubungi admin.")
        return None
    
    required_cols = ['Penjualan', 'Merk', 'Jumlah', 'Tanggal']
    if not all(col in df.columns for col in required_cols):
        print("[ERROR] Missing required columns")
        st.warning("Struktur data tidak sesuai. Kolom wajib hilang.")
        return None
    
    # --- FIX 3: HAPUS BARIS SAMPAH (TOTAL/SUBTOTAL) - SEKARANG CEK SEMUA KOLOM ---
    ...
    print(f"[DEBUG] After removing sampah rows: {len(df)} rows")  # Debug: setelah filter
        ...
        for target_brand, keywords in BRAND_ALIASES.items():
            for keyword in keywords:
                if keyword in raw_upper:
                    return target_brand
        print(f"[DEBUG] Unmatched brand: {raw_brand}")  # Debug: brand tidak match
        return raw_brand
    
    df['Merk'] = df['Merk'].apply(normalize_brand)
    
    # --- FIX 1: FORMAT ANGKA ANTI-ERROR (Hapus non-digit, handle desimal jika ada) ---
    # Asumsi: Jika ada koma sebagai desimal (e.g., 1500,50), replace koma ke titik dulu
    df['Jumlah'] = df['Jumlah'].astype(str).str.replace(',', '.', regex=False)  # Handle desimal Indo
    df['Jumlah'] = df['Jumlah'].str.replace(r'[^\d\.-]', '', regex=True)  # Hapus non-digit kecuali minus/desimal
    df['Jumlah'] = pd.to_numeric(df['Jumlah'], errors='coerce').fillna(0)
    
    # Tambah log untuk invalid numeric
    invalid_jumlah = df['Jumlah'].isna().sum()
    if invalid_jumlah > 0:
        print(f"[DEBUG] {invalid_jumlah} invalid Jumlah values set to 0")
    
    # Debug: Cek nilai kecil setelah cleaning
    small_values = df[(df['Jumlah'] > 0) & (df['Jumlah'] < 1000)]
    if not small_values.empty:
        print(f"[DEBUG] Found {len(small_values)} small values (<1000):")
        print(small_values[['Tanggal', 'Penjualan', 'Merk', 'Nama Barang', 'Jumlah']].to_string(index=False))
    
    sum_cleaned = df['Jumlah'].sum()
    print(f"[DEBUG] Total omset after cleaning: {sum_cleaned}")
    ...
    df = df.dropna(subset=['Tanggal', 'Penjualan', 'Merk', 'Jumlah'])  # Drop jika key cols NaN
    
    # Handle optional columns safely
    optional_cols = ['Kota', 'Nama Outlet', 'Nama Barang']
    for col in optional_cols:
        if col in df.columns:
            df[col] = df[col].astype(str).fillna('')
        else:
            df[col] = ''  # Default empty string jika hilang
...
def load_users():
    try:
        ...
        # Asumsi kolom: username, hashed_password, role, sales_name
        return df
    except:
        st.warning("File users.csv tidak ditemukan. Gunakan default atau hubungi admin.")
        return pd.DataFrame()
                ...
                if users.empty:
                    ...
                    if not match.empty:
                        ...
                        if bcrypt.checkpw(password.encode('utf-8'), stored_hash.encode('utf-8')):
                            ...
                            st.session_state['login_time'] = time.time()  # Untuk timeout
                            st.rerun()
                        else:
...
def main_dashboard():
    # Cek timeout session (e.g., 30 menit)
    if 'login_time' in st.session_state and time.time() - st.session_state['login_time'] > 1800:
        st.session_state['logged_in'] = False
        st.warning("Session timeout. Silakan login lagi.")
        st.rerun()
    ...
    if len(date_range) == 2:
        ...
        # Force default jika tidak valid
        start_date, end_date = default_start, default_end
        st.sidebar.warning("Rentang tanggal tidak valid, gunakan default bulan ini.")
    
    df_view_global = df[(df['Tanggal'].dt.date >= start_date) & (df['Tanggal'].dt.date <= end_date)]
       
    duration = (end_date - start_date).days + 1  # Hindari zero
    ...
    with tab1:
        ...
        if show_rapor_lengkap:
            ...
            st.dataframe(
                df_summary.style.apply(color_row, axis=1).hide_columns(subset=['Ach (Detail %)']),
            ...
            if brand_data:
                ...
                st.dataframe(
                    df_indiv.style.hide_columns(subset=['_pct_val']),
                    use_container_width=True, hide_index=True,
                    column_config={"Pencapaian": st.column_config.ProgressColumn("Bar", format=" ", min_value=0, max_value=1)}
                )
            else:
    ...
    with tab4:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Jumlah": st.column_config.NumberColumn("Omset", format="Rp %.0f")  # Handle float
            }
        )
