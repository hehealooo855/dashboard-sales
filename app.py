    ...
    with col2:
        with st.container(border=True):
            with st.form("login_form"):
                ...
                if submitted:
                    # --- SECURITY UPGRADE: Cek Login via st.secrets ---
                    try:
                        stored_password = st.secrets["users"][username]
                        # Disarankan menggunakan hashing, tapi untuk sekarang comparison string dulu
                        if stored_password == password:
                            
                            # Tentukan Role (Bisa diset di secrets atau logic sederhana ini)
                            role = "sales" # Default
                            if username in ["admin", "direktur", "manager"]:
                                role = "direktur"
                            elif username in ["fauziah"]:
                                role = "manager" # Atau akses khusus audit
                            ...
                            st.session_state['sales_name'] = username
...
def main_dashboard():
    ...
    # -- DATE PICKER PRESETS --
    c_1, c_2, c_3, c_4 = st.sidebar.columns(4)
    today = datetime.date.today()

    # 1. Yesterday
    if c_1.button("Kmrin", use_container_width=True):
        yesterday = today - datetime.timedelta(days=1)
        st.session_state['start_date'] = yesterday
        st.session_state['end_date'] = yesterday

    # 2. Last 7 Days
    if c_2.button("7Hari", use_container_width=True):
        st.session_state['start_date'] = today - datetime.timedelta(days=7)
        st.session_state['end_date'] = today

    # 3. This Month (Existing)
    if c_3.button("BulanIni", use_container_width=True):
        st.session_state['start_date'] = today.replace(day=1)
        st.session_state['end_date'] = today

    # 4. Last Month
    if c_4.button("BulanLalu", use_container_width=True):
    ...
    t1, t2, t_detail_sales, t3, t_rekap_toko, t4 = st.tabs(["📊 Rapor Brand", "📈 Tren Harian", "👥 Detail Tim", "🏆 Top Produk", "🏪 Rekap Toko", "📋 Data Rincian"])
    ...
    # --- NEW FEATURE: REKAP TOKO PER BRAND (BULANAN) ---
    with t_rekap_toko:
        ...
        if not df_active.empty:
            ...
            # Mapping Month Names to Indonesian (Optional safe approach without locale dependency)
            month_map = {
                1: 'Januari', 2: 'Februari', 3: 'Maret', 4: 'April', 5: 'Mei', 6: 'Juni',
                7: 'Juli', 8: 'Agustus', 9: 'September', 10: 'Oktober', 11: 'November', 12: 'Desember'
            }
            # Create sortable month key and display label
            ...
            # 5. Final Sorting & Ranking
            # Sort by Month desc, Brand asc, Store Total Sales desc
            final_rekap = final_rekap.sort_values(['MonthKey', 'Merk', 'Jumlah_Store'], ascending=[False, True, False])
            
            # Create Ranking per Brand per Month
