...
import datetime
import time
import re # Module regex untuk cleaning tingkat lanjut
...
def load_data():
    ...
    try:
        ...
        df = pd.read_csv(url_with_ts, dtype=str) # Read all as string to prevent auto-conversion errors
    ...
    # --- AGGRESSIVE CLEANING: REMOVE GHOST ROWS ---
    # Buang baris yang mengandung kata 'Total' di outlet/barang atau kosong
    if 'Nama Outlet' in df.columns:
        df = df[~df['Nama Outlet'].astype(str).str.contains('Total|Jumlah|Subtotal|Grand|Rekap', case=False, na=False)]
        df = df[df['Nama Outlet'].astype(str).str.strip() != ''] # Hapus outlet kosong
    
    if 'Nama Barang' in df.columns:
        df = df[~df['Nama Barang'].astype(str).str.contains('Total|Jumlah', case=False, na=False)]
        df = df[df['Nama Barang'].astype(str).str.strip() != ''] # Hapus barang kosong
    ...
    # 3. Logic Darurat: Jika angka < 1000, kali 1000 (Asumsi tidak ada penjualan < Rp 1000)
    ...
    def fix_swapped_date(d):
        ...
        try:
            # Jika Day <= 12 dan Day != Month, kemungkinan tertukar
            ...
            for brand in sales_brands:
                ...
                for spv, brands_dict in TARGET_DATABASE.items():
                    if brand in brands_dict:
                        target_found = brands_dict[brand]; spv_name = spv; break
