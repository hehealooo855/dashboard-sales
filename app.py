...
def main_dashboard():
    ...
    with t_kejar_omset:
        ...
        with col_k2:
            st.subheader("🧠 AI Market Basket Analysis (Cross-Sell Cerdas)")
            
            # 1. Prepare Basket (Outlet x Brand)
            # Use df_scope_all (historical data)
            basket = df_scope_all.groupby(['Nama Outlet', 'Merk'])['Jumlah'].sum().unstack().fillna(0)
            # Convert to 1 (bought) and 0 (not bought)
            basket_sets = basket.applymap(lambda x: 1 if x > 0 else 0)

            # 2. Mining Rules (Simplified)
            # We want to find: If Brand A -> then Brand B
            rules = []
            ...
            for antecedent in brands:
                for consequent in brands:
                    ...
                    # Count transactions
                    # Stores buying A
                    mask_a = basket_sets[antecedent] == 1
                    support_a = mask_a.sum()
                    
                    if support_a < 5: continue # Minimum support threshold
                    
                    # Stores buying A AND B
                    mask_ab = (basket_sets[antecedent] == 1) & (basket_sets[consequent] == 1)
                    support_ab = mask_ab.sum()
                    
                    confidence = support_ab / support_a
                    
                    if confidence >= 0.5: # 50% threshold
                        rules.append({
                            "Jika Beli": antecedent,
                            "Maka Beli": consequent,
                            "Confidence": confidence,
                            "Support": support_a
                        })
            
            # 3. Generate Recommendations
            # For every rule (A -> B), find stores buying A but NOT B
            recommendations = []
            for rule in rules:
                ant = rule['Jika Beli']
                cons = rule['Maka Beli']
                ...
                # Find target stores
                target_stores = basket_sets[
                    (basket_sets[ant] == 1) & (basket_sets[cons] == 0)
                ].index.tolist()
                
                for store in target_stores:
                     # Get Salesman name
                     pic = df_scope_all[df_scope_all['Nama Outlet'] == store]['Penjualan'].iloc[0]
                     recommendations.append({
                         "Nama Toko": store,
                         "Sales": pic,
                         "Rekomendasi": f"Tawarkan {cons} (Karena beli {ant})",
                         "Confidence": f"{rule['Confidence']*100:.0f}%"
                     })
            
            if recommendations:
                st.info(f"Ditemukan **{len(recommendations)} Peluang Cross-Sell** berdasarkan pola pembelian toko lain.")
                df_mba = pd.DataFrame(recommendations)
                st.dataframe(df_mba, use_container_width=True, hide_index=True)
            else:
                st.info("Belum cukup data untuk menemukan pola pembelian yang kuat.")
