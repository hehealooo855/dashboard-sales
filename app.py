...
def render_pivot_fragment(df_scope_all, role):
    ...
    # --- 🚀 MENGATASI BUG NAMA CUSTOMER HILANG ---
    ...
    if not master_pivot.empty:
        ...
        # --- PASTIKAN TIDAK ADA DUPLIKAT SAAT RENAME ---
        ren_dict = {}
        ...
        if not df_filtered.empty:
            # --- 🪚 PEMUSNAH KOLOM GANDA ---
            df_filtered = df_filtered.loc[:, ~df_filtered.columns.duplicated()]
            if 'Nama Outlet' in df_filtered.columns and 'Nama Customer' in df_filtered.columns:
                df_filtered = df_filtered.drop(columns=['Nama Outlet'])
            ...
            if AGGRID_AVAILABLE:
                # Pisahkan Total dari Data Utama untuk di-Pin
                df_clean = df_filtered.copy()
                
                gb = GridOptionsBuilder.from_dataframe(df_clean)
                
                # --- 🔍 FILTER CORONG + FLOATING KETIK CEPAT ---
                ...
                for col in df_clean.columns:
                    if col in num_cols:
                        gb.configure_column(col, type=["numericColumn", "numberColumnFilter"], valueFormatter=rupiah_format)
        ...
        with tab_sku:
            ...
            if not filter_nama_sku and not filter_sku_spesifik:
                ...
                if not df_sku_filtered.empty:
                    ...
                    pivot_sku = pivot_sku.rename(columns={'Pivot_Index': display_label})
                    
                    bulan_indo_map = {1: 'Januari', 2: 'Februari', 3: 'Maret', 4: 'April', 5: 'Mei', 6: 'Juni', 7: 'Juli', 8: 'Agustus', 9: 'September', 10: 'Oktober', 11: 'November', 12: 'Desember'}
                    for i in range(1, 13):
                        if i not in pivot_sku.columns: pivot_sku[i] = 0
                        
                    cols_sku = [display_label] + list(range(1, 13))
                    ...
                    if AGGRID_AVAILABLE:
                        ...
                        for col in df_clean_sku.columns:
                            if col in num_cols_sku:
                                ...
                                gb_sku.configure_column(col, pinned='left')
                        ...
                        st.info("💡 **Tips:** Ketik di kotak putih di bawah judul kolom untuk memfilter data dengan cepat.")
                        
                        AgGrid(
                            df_clean_sku,
                            gridOptions=go_sku,
                            theme='balham',
                            height=800 if maximize_toggle_sku else 600,
                            allow_unsafe_jscode=True,
                            enable_enterprise_modules=True,
...
def render_pivot_fragment(df_scope_all, role):
    ...
    # BUNGKUS SEMUA FILTER KE DALAM "FORM"
    with st.form(key='pivot_filter_form'):
        col_piv1, col_piv2 = st.columns(2)
        with col_piv1:
            selected_merk_excel = st.selectbox("🎯 Pilih Merk:", ["SEMUA"] + list_merk_excel)
    ...
    if not master_pivot.empty:
        ...
        if not df_filtered.empty:
            ...
            # 📌 PEMBUATAN GRAND TOTAL UNTUK PINNED BOTTOM
            total_dict = {col: "" for col in df_filtered.columns}
            gt_col = 'Nama Customer' if 'Nama Customer' in df_filtered.columns else df_filtered.columns[0]
            total_dict[gt_col] = "GRAND TOTAL" 
            ...
            if AGGRID_AVAILABLE:
                ...
                # 🔍 FILTER CORONG (GAYA A: FLOATING FILTER AKTIF & MENU EXCEL)
                gb.configure_default_column(
                    filter='agSetColumnFilter', 
                    sortable=True, 
                    resizable=True, 
                ...
                # 📌 KUNCI GRAND TOTAL DI BAWAH (PINNED)
                go['pinnedBottomRowData'] = [total_dict]
                go['getRowStyle'] = JsCode("""
                function(params) {
                    if (params.node.rowPinned) {
                ...
                # 🎨 TEMA CORPORATE BLUE (HEADER BIRU)
                custom_css = {
                    ".ag-header": {"background-color": "#2980b9 !important", "border-bottom": "none !important"},
                    ".ag-header-row": {"background-color": "#2980b9 !important"},
                    ".ag-header-cell-text": {"color": "white !important", "font-weight": "bold !important"},
                    ".ag-icon": {"color": "white !important"},
                    ".ag-theme-balham .ag-filter-toolpanel-body": {"background-color": "white !important", "color": "black !important"}
                }
                ...
                df_export_fallback = pd.concat([df_display, pd.DataFrame([total_dict])], ignore_index=True)
                st.warning("Library st_aggrid tidak ditemukan! Menggunakan tabel bawaan Streamlit.")
                st.dataframe(df_export_fallback)
        else:
            st.info("Data Kosong setelah difilter.")
        ...
        if user_role_lower in ['direktur', 'manager', 'supervisor']:
            if 'df_display' in locals() and not df_display.empty:
                output = io.BytesIO()
                with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                    df_export_final = pd.concat([df_display, pd.DataFrame([total_dict])], ignore_index=True)
                    df_export_final.to_excel(writer, index=False, sheet_name='Master Data')
                    ...
                    bold_format = workbook.add_format({'bold': True, 'bg_color': '#f2f2f2', 'border': 1, 'num_format': '#,##0'})
                    last_row_idx = len(df_export_final) 
                    worksheet.set_row(last_row_idx, None, bold_format)
